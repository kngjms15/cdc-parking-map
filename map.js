(function () {
  ...
  if (!frame || !content || !parking) return;
  ...
  function getBaseScale() {
    ...
    // SharePoint/iframes can report 0 briefly → don't allow scale(0)
    if (!fw || !fh) return null;

    const s = Math.min(fw / DESIGN_W, fh / DESIGN_H);
    // extra safety: never return 0/NaN/Infinity
  ...
  function setBaseAndFit() {
    ...
    // first run OR after a bad resize
    if (!scale || scale < minScale) scale = minScale;
  ...
  // === INIT (robust for SharePoint timing) ===
  function init() {
    // try immediately
    if (setBaseAndFit()) return;

    // retry a few times (SharePoint embeds often size late)
    let tries = 0;
    const retry = setInterval(() => {
      tries++;
      if (setBaseAndFit() || tries >= 30) clearInterval(retry);
    }, 150);
  }

  window.addEventListener('load', init);
  window.addEventListener('resize', () => {
    // avoid applying scale(0) during transient resize
    setBaseAndFit();
  });
